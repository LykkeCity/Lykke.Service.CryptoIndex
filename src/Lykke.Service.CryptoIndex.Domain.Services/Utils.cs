...
namespace Lykke.Service.CryptoIndex.Domain.Services
{
    public static class Utils
    {
        ...
        public static IReadOnlyList<string> GetNewAssets(IReadOnlyList<string> whiteAndIgnoredAssets, IReadOnlyList<AssetMarketCap> allMarketCaps, int topAssetsToCheck)
        {
            var absentAssets = new List<string>();
            for (int i = 0; i < topAssetsToCheck; i++)
            {
